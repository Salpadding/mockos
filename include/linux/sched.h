...
struct tss_struct {
   ...
   * esp0 = 00025c80
   * ss0 = 0x10
   * esp1=0,ss1=0,esp2=0,ss2=0,cr3=0, eip = 0
   *
   * */
  long back_link; /* 16 high bits zero */ // 0
  long esp0;
  long ss0; /* 16 high bits zero */ // 8
  long esp1;
  long ss1; /* 16 high bits zero */ // 16
  long esp2;
  long ss2; /* 16 high bits zero */ // 24
  long cr3;
  long eip; // 这个 eip 实际上用不到, idt 中才是真正的 eip
  long eflags;
...
struct task_struct {
  ...
  long blocked;
      /* 进程信号屏蔽码(对应信号位图) */ /* bitmap of masked signals */
  ...
  struct file
      *filp[NR_OPEN]; /* 文件结构指针表，最多32项。表项号即是文件描述符的值 */
...
#define INIT_TASK                                                              \
  /* state etc */ {                                                            \
    0, 15, 15, /* signals */ 0,                                                \
        ...
        0, /* ec,brk... */ 0, 0, 0, 0, 0, 0, /* pid etc.. */ 0, 0, 0, 0,       \
        /* suppl grps*/                                                        \
        {                                                                      \
            NOGROUP,                                                           \
        },                                                                     \
        /* proc links*/ &init_task.task, 0, 0, 0, /* uid etc */ 0, 0, 0, 0, 0, \
        0, /* timeout */ 0, 0, 0, 0, 0, 0, 0,                                  \
        /* rlimits */ {{0x7fffffff, 0x7fffffff}, {0x7fffffff, 0x7fffffff},     \
                       {0x7fffffff, 0x7fffffff}, {0x7fffffff, 0x7fffffff},     \
                       {0x7fffffff, 0x7fffffff}, {0x7fffffff, 0x7fffffff}},    \
        /* flags */ 0, /* math */ 0, /* fs info */ -1, 0022, NULL, NULL, NULL, \
        NULL, 0, /* filp */                                                    \
        {                                                                      \
            NULL,                                                              \
        },                                                                     \
...
extern void set_base(void* p, unsigned long base);
...
extern unsigned long get_base(void* p);
